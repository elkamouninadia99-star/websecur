...
export default function Features() {
  return <section id="features" className="section bg-[#081321]"><div className="container"><div className="max-w-2xl"><span className="eyebrow">The WebSecur standard</span><h2 className="section-title">A better foundation for everything you build online.</h2><p className="section-copy">Design, performance and protection should work together. We make the essentials feel effortless.</p></div><div className="mt-10 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{items.map(([Icon, title, copy]) => <article key={title} className="surface rounded-2xl p-5 transition-transform duration-200 hover:-translate-y-1"><Icon className="size-5 text-cyan-200"/><h3 className="mt-5 font-semibold text-white">{title}</h3><p className="mt-2 text-sm leading-6 text-slate-400">{copy}</p></article>)}</div></div></section>;
}
